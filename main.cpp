...
int main(int argc, char *argv[])
{
    ...
    int methodIndex = m->metaObject()->indexOfMethod("hello(QString)");
    QMetaMethod method = m->metaObject()->method(methodIndex);
    method.invoke(m, Qt::QueuedConnection, Q_ARG(QString, "arg"));

    QSlimServer slim;
    SlimDeserializer *ser=0;
    try {
